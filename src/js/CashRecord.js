...
import {Message, Segment, Grid, Table, Icon, Form, Input, Label,  Header, Divider, Dropdown, Button} from 'semantic-ui-react'
...
class CashRecord extends Component {
                        ...
                        <Divider />
                            <Table basic='very' celled >
                                <Table.Body>
                                    ...
                                    <Table.Row>                                   
                                   <Table.Cell><b> Choose Account </b></Table.Cell>
                                   <Table.Cell>
                                   <Form.Field error={!!errors.accountChargedId}>
                                       ...
                                       />
                                       {errors.accountChargedId && <InlineError text={errors.accountChargedId} />}
                                       </Form.Field>
                                    </Table.Cell>
                                   </Table.Row>  
                                                                 
                                   <Table.Row>                                   
                                   <Table.Cell><b> Enter Amount </b></Table.Cell>
                                   <Table.Cell>
                                   <Form.Field error={!!errors.amountRecieved}>
                                   <Input fluid labelPosition='right' type='number'
                                   onChange={e=>this.setState({ amountRecieved: e.target.value })}
                                    placeholder='Amount'>
                                        <Label basic>KES</Label>
                                        <input />
                                        <Label>.00</Label>
                                    </Input> 
                                    {errors.amountRecieved && <InlineError text={errors.amountRecieved} />}
                                    </Form.Field>
                                   </Table.Cell>
                                   </Table.Row>
    ...
    _addBalance = async()=>{
        ...
        await this.props.addBalance({
            variables:{accountChargedId, amountRecieved, dateIssued, balBefore, CashBalId}
        });
...
mutation addBalance( $accountChargedId:ID!, 
  $CashBalId:ID!, $balBefore:Int!, $dateIssued:DateTime! 
  $amountRecieved:Int!) {
  addCashReceipt(accountChargedId: $accountChargedId,
   CashBalId:$CashBalId, dateIssued:$dateIssued,
  balBefore:$balBefore,
