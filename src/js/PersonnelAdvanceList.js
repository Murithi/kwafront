...
import {Message, Icon, Segment, Form, Table, Header, Tab, Menu} from 'semantic-ui-react'
import { Link } from 'react-router-dom'
import moment from 'moment'
import AdvancesRecieved from './queries/fetchAdvancesRecieved'
import {graphql} from 'react-apollo';


class PersonnelAdvanceList extends Component {
    constructor(props) {
        super(props);
        this.state = {  }
    }
    render() { 
        console.log(this.props.advanceFeed)
        const loadingMessage = (
          ...
          const timeoutMessage = (
            <Message icon negative>
                <Icon name="wait" />
                <Message.Content>
                    <Message.Header>{this.props.advanceFeed.errorl}</Message.Header>
                    Is the backend server running?
              </Message.Content>
            </Message>
        );

        if (this.props.advanceFeed && this.props.advanceFeed.loading) {
            return <div>{loadingMessage}</div>;
          }
      
          if (this.props.advanceFeed && this.props.advanceFeed.error) {
            return <div>{timeoutMessage}</div>;
          }
      
          if (this.props.advanceFeed === undefined ||this.props.requisitionFeed === 0) {
        ...
        <Fragment>       
            ...
            <Table celled selectable>
                ...
                <Table.Body>
                {
                    this.props.advanceFeed.personnel.advancesRecieved.map(request => (
                        <Table.Row key={request.payee.id}>
                            ...
                            <Table.Cell>{moment(request.cashIssueDetails.dateIssued).format('MMM DD YYYY')}</Table.Cell>
                        </Table.Row>
                    ))
                }
                </Table.Body>
...
export default graphql(AdvancesRecieved, {
    name:'advanceFeed',
    options: props=>({
        variables:{
