...
class PaymentListing extends Component {
	...
	render() {
		...
		if (
			this.props.paymentFeed.paymentIssueFeed === undefined ||
			this.props.paymentFeed.paymentIssueFeed === 0
		) {
				...
				<Table.Body>
					{this.props.paymentFeed.paymentIssueFeed.map(request => (
						<Table.Row>
							...
							<Table.Cell>
								{request.issuedBy.personnelDetails.firstName}{' '}
								{request.issuedBy.personnelDetails.lastName}
							</Table.Cell>
							<Table.Cell>
