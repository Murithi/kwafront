...
class InspectionRequestCreate extends Component {
	...
	validate = () => {
		...
		if (!this.state.approxCostOfInspection)
			errors.approxCostOfInspection = "Can't be blank"
						...
						<Segment stacked>
							...
							<Form.Field error={!!errors.approxCostOfInspection}>
								...
								<input
									...
									type="number"
									onChange={e =>
										this.setState({ approxCostOfInspection: e.target.value })
									}
								/>
								{errors.approxCostOfInspection && (
									<InlineError text={errors.approxCostOfInspection} />
								)}
							</Form.Field>
	...
	_createVehicleInspection = async () => {
		const { otherDetails, approxCostOfInspection, vehicleId } = this.state
