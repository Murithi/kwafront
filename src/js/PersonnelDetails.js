...
class PersonnelDetails extends Component {
	...
	render() {
		const panes = [
			...
			{
				menuItem: 'Salary History',
				render: () => (
					<Tab.Pane attached={false}>
						<PersonnelSalariesRecieved id={this.props.match.params.id} />
					</Tab.Pane>
				),
			},
			{
				menuItem: 'Advances History',
				render: () => (
					<Tab.Pane attached={false}>
						<PersonnelAdvancesRecieved id={this.props.match.params.id} />
					</Tab.Pane>
				),
			},
		]
