...
class SalaryRequestsInitiated extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
  render() {
    ...
    if (
      this.props.requisitionFeed.initiatedSalaryRequestFeed === undefined ||
      this.props.requisitionFeed.initiatedSalaryRequestFeed === 0
    ) {
    ...
    return (
      <Fragment>
        <Header as="h4" color="green" textAlign="center">
          Initiated Salary Requests List
        </Header>
        {(() => {
        ...
        <Table celled selectable>
          ...
          <Table.Body>
            {this.props.requisitionFeed.initiatedSalaryRequestFeed.map(
              request => (
                <Table.Row>
                  <Table.Cell>
                  ...
                  <Table.Cell>
                    {(() => {
                      if (request.approvalStatus) {
                        ...
                        if (this.props.userDetails.me.role === "DIRECTOR") {
                          return (
                            <Link to={`/salaryrequests/approve/${request.id}`}>
                              Approve
  ...
  _approveRequest = async (approvalStatus, salaryrequestId) => {
...
const APPROVEALL = gql`
  mutation approveAll($approvalDate: String!) {
    approveAllSalaryRequest(approvalDate: $approvalDate)
  }
`;
