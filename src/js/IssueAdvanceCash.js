...
var accountOptions = [];
var projectOptions = [];
class IssueAdvanceCash extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            amountRequested: '',
                            ...
                            <Divider />
                                <Table basic='very' celled >
                                    ...
                                    <Table.Row>
                                        ...
                                        <Table.Cell>{initiatedAdvanceRequest.payee.firstName} &nbsp; {initiatedAdvanceRequest.payee.lastName}</Table.Cell>
                                        </Table.Row>
                                        <Table.Row>
                                        <Table.Cell><b>Requested By</b></Table.Cell>
                                        <Table.Cell>{initiatedAdvanceRequest.requestedBy.personnelDetails.firstName} &nbsp; {initiatedAdvanceRequest.requestedBy.personnelDetails.lastName}</Table.Cell>
                                        </Table.Row>  
                                        <Table.Row>
                                        <Table.Cell><b>Amount Requested </b></Table.Cell>
                                        <Table.Cell>{initiatedAdvanceRequest.amountRequested}</Table.Cell>
                                        </Table.Row> 
                                        <Table.Row>                                   
                                   ...
                                   <Table.Cell>
                                       ...
                                       />
                                        ...
                                        <Table.Cell>{moment(initiatedAdvanceRequest.requestDate).format('MMM Do YYYY')} </Table.Cell>
                                        </Table.Row>
                                        <Table.Row>
                                        <Table.Cell><b>Date Approved </b></Table.Cell>
                                        <Table.Cell>{moment(initiatedAdvanceRequest.approvalDate).format('MMM Do YYYY')} </Table.Cell>
                                        </Table.Row>
                                        <Table.Row>
                                        <Table.Cell><b>Other Details </b></Table.Cell>
                                        <Table.Cell>{initiatedAdvanceRequest.otherDetails}</Table.Cell>
                                        </Table.Row>
                                        <Table.Row>
                                            <Table.Cell><b>Amount Issuing</b></Table.Cell>
                                            <Table.Cell>
                                                <Input
                                                value={initiatedAdvanceRequest.amountRequested}
                                ...
                                <Button
                                    attached='bottom'
                                    positive
                                    onClick={()=>this._issueCash(initiatedAdvanceRequest.id, initiatedAdvanceRequest.amountRequested)}
                                >Issue Cash</Button>
    ...
    _issueCash = async (requestId, amountIssued) => {
        ...
        let paymentType='ADVANCEREQUESTS'
        ...
        await this.props.issuePayment({
            ...
            refetchQueries: [{query: ApprovedRequisitionsQuery}]
        })
        this.props.history.push('/advancerequests/approved')
        
     }
}


const ADDADVANCEPAYMENTISSUE = gql`
...
export default compose(
    ...
    graphql(ADDADVANCEPAYMENTISSUE,
        {
            name:'issuePayment'
        } 
    ),
