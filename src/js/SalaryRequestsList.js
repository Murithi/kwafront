...
import getSalaryRequisitions from './queries/fetchSalaryRequestsList'


class SalaryRequestsList extends Component {
    constructor(props) {
        super(props);
        this.state = {  }
    }
    render() { 
        const loadingMessage = (
          ...
          if (this.props.requisitionFeed && this.props.requisitionFeed.loading) {
            ...
            if (this.props.requisitionFeed.salaryRequestFeed === undefined ||this.props.requisitionFeed.salaryRequestFeed === 0) {
        ...
        return ( 
            ...
            <Table celled selectable>
                ...
                {
                    this.props.requisitionFeed.salaryRequestFeed.map(request => (
                        <Table.Row>
                        ...
                        {(() => { 
                          if (request.cashIssueDetails !== null) { 
                            return (
                              <Table.Cell>{moment(request.cashIssueDetails.dateIssued).format('MMM Do YYYY')}</Table.Cell>
                            )
                          }
                          else {
                            <Table.Cell><React.Fragment /></Table.Cell>
...
export default graphql(getSalaryRequisitions, 
    { 
