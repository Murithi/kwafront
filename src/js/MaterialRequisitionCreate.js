...
class MaterialRequisitionCreate extends Component {
	...
	handleOpen = () => {
		// t
		let supplier = this.props.supplierFeed.suppliersFeed.find(
			x => x.id === this.state.supplierId,
		)
	...
	render() {
		...
		const paymentOptions = [
			{ id: 1, value: 'CASH', text: 'CASH' },
			{ id: 2, value: 'CHEQUE', text: 'CHEQUE' },
		]
		console.log(this.props.supplierFeed)

		if (this.props.supplierFeed.loading === false) {
			if (
				this.props.supplierFeed.suppliersFeed !== undefined ||
				this.props.supplierFeed.suppliersFeed.length !== 0
			) {
				let tempOp = [...new Set(this.props.supplierFeed.suppliersFeed)]
				supplierOptions = []
				tempOp.map(element => {
					return supplierOptions.push({
						id: element.id,
						text: element.material.materialName + ' ' + element.supplierName,
						value: element.id,
					})
				})
			}
			...
			>
				<Grid.Column style={{ maxWidth: 600 }}>
						...
						<Segment stacked>
							<Form.Field error={!!errors.materialTypeId}>
								...
								{errors.materialTypeId && (
									<InlineError text={errors.materialTypeId} />
								)}
							</Form.Field>
							...
							<Form.Field error={!!errors.paymentMode}>
								...
								/>
								{errors.paymentMode && (
									<InlineError text={errors.paymentMode} />
								)}
							</Form.Field>
					...
					>
						...
						<Modal.Content>
							<b>Material: {this.state.material}</b>
							<br />
							<b>Supplier: {this.state.supplier}</b>
							<br />
							<b>Quantity: {this.state.quantity}</b>
							<br />
							<b>Amount Charged: {this.state.approxCost}</b>
							<br />
							<p>
								Please confirm the cost & supplier chosen for the transaction
							</p>
						</Modal.Content>
						<Modal.Actions>
							<Button
