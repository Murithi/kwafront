...
var accountOptions = [];
var projectOptions = [];
class IssueInspectionCash extends Component {
  constructor(props) {
    super(props);
    this.state = {
      amountRequested: ""
  ...
  render() {
    ...
    if (this.props.inspectionRequest.error) {
      return <div>{timeoutMessage}</div>;
    }

    const { initiatedVehicleInspection } = this.props.inspectionRequest;

    if (!initiatedVehicleInspection) {
              ...
              <Table basic="very" celled>
                <Table.Body>
                  ...
                  <Table.Row>
                    ...
                    <Table.Cell>
                      {moment(initiatedVehicleInspection.approvalDate).format(
                        "MMM Do YYYY"
                      )}{" "}
                    </Table.Cell>
                  </Table.Row>
                  <Table.Row>
                    ...
                    <Table.Cell>
                      {initiatedVehicleInspection.otherDetails}
                    </Table.Cell>
                  </Table.Row>
                  <Table.Row>
                    <Table.Cell>
  ...
  _issueCash = async (requestId, amountIssued) => {
    ...
    let paymentType = "INSPECTIONREQUESTS";
    ...
    this.props.history.push("/inspectionrequisitions/approved");
  };
}

const ADDPAYMENTDETAILS = gql`
  mutation issueInspectionPayment(
    $accountchargedId: ID!
    $cashbalId: ID!
    $balbefore: Int!
    $otherDetails: String
