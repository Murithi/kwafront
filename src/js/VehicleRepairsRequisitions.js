...
class VehicleRepairsRequisitions extends Component {
	constructor(props) {
		super(props)
		this.state = {}
	}
	render() {
		if (this.props.repairdetail.loading) {
			return <div>{loadingMessage}</div>
		}
		if (this.props.repairdetail.loading) {
			return <div>{timeoutMessage}</div>
		}
		console.log(this.props.repairdetail)
		const { getVehicle } = this.props.repairdetail
...
`
export default graphql(GETREPAIRSDETAILS, {
	name: 'repairdetail',
	options: props => ({
		variables: {
			id: props.id,
