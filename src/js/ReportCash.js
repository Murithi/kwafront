...
class ReportCash extends Component {
    ...
    render() {
        let requestServiceId = this.props.match.params.id;
        ...
        const { issuedRequisition } = this.props.paymentDetails
        if (!issuedRequisition) {
            return <div>{emptyMessage}</div>
         }else{
            
             paymentIssue_id = issuedRequisition.paymentsDetails.id
            }
       return (
                    ...
                    <Segment.Group horizontal>
                        ...
                        <Divider />
                                <Table basic='very' celled >
                                            ...
                                            <Table.Cell>{issuedRequisition.requestedBy.personnelDetails.firstName} &nbsp; {issuedRequisition.requestedBy.personnelDetails.lastName}</Table.Cell>
                                                </Table.Row>
                                                <Table.Row>
                                        <Table.Cell><b>Amount Issued </b></Table.Cell>
                                        <Table.Cell>KES {issuedRequisition.paymentsDetails.amountIssued}</Table.Cell>
                                                </Table.Row> 
                                                <Table.Row>
                                        <Table.Cell><b>Vehicle Serviced </b></Table.Cell>
                                        <Table.Cell>{issuedRequisition.vehicleToBeServiced.registrationNumber}</Table.Cell>
                                        </Table.Row>
                                                <Table.Row>
                                        <Table.Cell><b>Details </b></Table.Cell>
                                        <Table.Cell>{issuedRequisition.otherDetails}</Table.Cell>
                                        </Table.Row>        
                                            </Table.Body>
                                        </Table>   
                                        <Divider /> 
    ...
    graphql(getPaymentDetails,
        {
            ...
            options: props=>({
                variables:{
                    requestServiceId: props.match.params.id
                }
            })
        }
    )
