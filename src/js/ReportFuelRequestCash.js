...
import getPaymentDetails from './queries/fetchFuelPaymentDetails';
...
const loadingMessage = (
  ...
  class ReportFuelRequestCash extends Component {
      constructor(props) {
          super(props);
          this.state = { 
            errors: {},
    ...
    onSubmit = () => {
      ...
      render() { 
        ...
        const { issuedFuelRequisition } = this.props.paymentIssue
        if (issuedFuelRequisition.length === 0) {
            return <div>{emptyMessage}</div>
         }else{
            console.log(issuedFuelRequisition.paymentsDetails.id)
            paymentIssue_id = issuedFuelRequisition.paymentsDetails.id
           }
         return ( 
...
<Divider />
      <Table basic='very' celled >
                  ...
                  <Table.Cell>{issuedFuelRequisition.requestedBy.personnelDetails.firstName} &nbsp; {issuedFuelRequisition.requestedBy.personnelDetails.lastName}</Table.Cell>
                      </Table.Row>
                      <Table.Row>
              <Table.Cell><b>Amount Issued </b></Table.Cell>
              <Table.Cell>KES {issuedFuelRequisition.paymentsDetails.amountIssued}</Table.Cell>
                      </Table.Row> 
                      <Table.Row>
              <Table.Cell><b>Vehicle Serviced </b></Table.Cell>
              <Table.Cell>{issuedFuelRequisition.vehicleToBeFueled.registrationNumber}</Table.Cell>
              </Table.Row>
                      <Table.Row>
              <Table.Cell><b>Details </b></Table.Cell>
              <Table.Cell>{issuedFuelRequisition.otherDetails}</Table.Cell>
              </Table.Row>        
                  </Table.Body>
              </Table>   
              <Divider /> 
      ...
      _addPaymentDetails = async () => {
        ...
        this.props.history.push('/fuelrequisitions/issued');
    }
  }
