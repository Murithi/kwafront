...
class SalaryRequestsApproved extends Component {
    constructor(props) {
        super(props);
        this.state = {  }
    }
    render() { 
        const loadingMessage = (
          ...
          if (this.props.requisitionFeed.approvedSalaryRequestFeed === undefined ||this.props.requisitionFeed.approvedSalaryRequestFeed === 0) {
            return <div>{emptyMessage}</div>;
          }
        return ( 
            <Fragment>
            <Header as="h4" color="green" textAlign="center">
                Approved Salary Requests List
            </Header>
            <Table celled selectable>
                ...
                {
                    this.props.requisitionFeed.approvedSalaryRequestFeed.map(request => (
                        <Table.Row>
                        ...
                        if (this.props.userDetails.me.role === 'ACCOUNTANT') {
                            return (
                                <Link to={`/salaryrequests/issue/${request.id}`}>
                                    Issue Cash
