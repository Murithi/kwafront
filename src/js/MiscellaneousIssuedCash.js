...
class MiscellaneousIssuedCash extends Component {
	...
	render() {
		...
		if (
			this.props.requisitionFeed.issuedMiscellaneousRequisitionsFeed ===
				undefined ||
		...
		return (
			<React.Fragment>
				...
				<Table celled selectable>
					...
					<Table.Body />
					{this.props.requisitionFeed.issuedMiscellaneousRequisitionsFeed.map(
						request => {
							console.log(request.cashPaymentsDetails.amountCharged)
							if (
								request.issuedCash === true &&
								request.cashPaymentsDetails.amountCharged === null
							) {
								return (
									<Table.Row>
