...
class AssignPersonnelProject extends Component {
	...
	validate = () => {
		...
		if (!this.state.projectAssignedId)
			errors.projectAssignedId = "Can't be blank"
					...
					<Grid.Column style={{ maxWidth: '100%' }}>
						...
						<Segment.Group horizontal loading={loading}>
							<Segment>
								<React.Fragment>
									<Image
										src={searchedPersonnelDetails.photoUrl}
										size="medium"
										centered
									/>
								</React.Fragment>
								...
								<Form size="mini" onSubmit={this.onSubmit}>
									...
									<Segment>
										<Form.Field error={!!errors.projectAssignedId}>
											...
											/>
											{errors.projectAssignedId && (
												<InlineError text={errors.projectAssignedId} />
											)}
										</Form.Field>
									</Segment>
...
const ASSIGNPERSONNELPROJECT = gql`
	mutation assignEmployeeProject($id: ID!, $projectId: ID!) {
		assignToProject(id: $id, projectId: $projectId)
	}
`
