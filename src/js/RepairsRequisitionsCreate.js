...
class RepairsRequisitionsCreate extends Component {
	...
	validate = () => {
		...
		if (!this.state.vehicleToBeRepaired)
			errors.vehicleToBeRepaired = "Can't be blank"
		if (!this.state.approxCostOfRepair)
			errors.approxCostOfRepair = "Can't be blank"
						...
						<Segment stacked>
							...
							<Form.Field error={!!errors.approxCostOfRepair}>
								...
								<input
									...
									type="number"
									onChange={e =>
										this.setState({ approxCostOfRepair: e.target.value })
									}
								/>
								{errors.approxCostOfRepair && (
									<InlineError text={errors.approxCostOfRepair} />
								)}
							</Form.Field>
							...
							<Form.Field error={!!errors.requestDate}>
								...
								<DatePicker
									...
									onChange={this.handleRepairsDateChange}
								/>
							</Form.Field>
	...
	_createRepairsRequisition = async () => {
		const { otherDetails, vehicleToBeRepaired, approxCostOfRepair } = this.state
...
export default compose(
	...
	graphql(CREATEREPAIRSREQUISITIONMUTATION, {
		name: 'createRepairsRequisition',
	}),
