...
class VehicleServiceRequisitions extends Component {
	constructor(props) {
		super(props)
		this.state = {}
	}
	render() {
		if (this.props.servicedetail.loading) {
			return <div>{loadingMessage}</div>
		}
		if (this.props.servicedetail.loading) {
			return <div>{timeoutMessage}</div>
		}
		console.log(this.props.servicedetail)
		const { getVehicle } = this.props.servicedetail
...
`
export default graphql(GETSERVICEDETAILS, {
	name: 'servicedetail',
	options: props => ({
		variables: {
			id: props.id,
