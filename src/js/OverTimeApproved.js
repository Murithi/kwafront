...
class OverTimeApproved extends Component {
    constructor(props) {
        super(props);
        this.state = {  }
    }
    ...
    render() { 
        const loadingMessage = (
          ...
          if (this.props.requisitionFeed.approvedOverTimeRequestFeed === undefined ||this.props.requisitionFeed.approvedOverTimeRequestFeed === 0) {
            return <div>{emptyMessage}</div>;
          }
        return ( 
            <Fragment>
            <Header as="h4" color="green" textAlign="center">
                Approved Overtime Requests List
            </Header>
            <Table celled selectable>
                ...
                {
                    this.props.requisitionFeed.approvedOverTimeRequestFeed.map(request => (
                        <Table.Row>
                        ...
                        if (this.props.userDetails.me.role === 'ACCOUNTANT') {
                            return (
                                <Link to={`/overtimerequests/issue/${request.id}`}>
                                    Issue Cash
