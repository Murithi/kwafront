...
const StoreIssuesDisplay = (props) => {
    ...
    return ( 
        ...
        <Header as="h3" color="green" textAlign="center">
                    Store { (props.transactionList.storageFeed[0].transactionType==="ISSUES") ? "Issues" : "Receipts"} List
            </Header> 
            <Table>
            <Table.Header>
                <Table.Row>
                    ...
                    <Table.HeaderCell>
                    { (props.transactionList.storageFeed[0].transactionType==="ISSUES") ? "Issued To" : "Materials From"}
                    </Table.HeaderCell>

                    <Table.HeaderCell>Quantity Before</Table.HeaderCell>
                   
                    <Table.HeaderCell> { (props.transactionList.storageFeed[0].transactionType==="ISSUES") ? "Quantity Issued" : "Quantity Recieved"} </Table.HeaderCell>
            ...
            <Table.Body>
                {
                   props.transactionList.storageFeed.map(request =>{
                      return( 
                       <Table.Row>
                           <Table.Cell> {request.itemTransacted.materialName} </Table.Cell>
                           <Table.Cell> { (request.transactionType==="ISSUES") ? request.materialsIssuedTo.projectName : request.materialsRecievedFrom.materialName} </Table.Cell>
                           ...
                           <Table.Cell> {moment(request.transactionDate).format('MMM DD YYYY')} </Table.Cell>
                       </Table.Row>
                      )
                   }) 
                }
