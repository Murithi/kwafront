        ...
        const loadingMessage = (
          ...
          const timeoutMessage = (
            <Message icon negative>
              <Icon name="wait" />
              <Message.Content>
                <Message.Header>{this.props.assignedVehiclesFeed.errorl}</Message.Header>
                Is the backend server running?
              </Message.Content>
            </Message>
          );
          if (this.props.assignedVehiclesFeed && this.props.assignedVehiclesFeed.loading) {
            return <div>{loadingMessage}</div>;
          }
      
          if (this.props.assignedVehiclesFeed && this.props.assignedVehiclesFeed.error) {
            return <div>{timeoutMessage}</div>;
          }
      
          if (this.props.assignedVehiclesFeed.vehicleAssignmentFeed.length === 0 || this.props.assignedVehiclesFeed.vehicleAssignmentFeed === null) {
        ...
        return (
            ...
            <Table.Header>
              <Table.Row>
                ...
                <Table.Body>
                    {this.props.assignedVehiclesFeed.vehicleAssignmentFeed.map(vehicle => {
                   return( <Table.Row>
                        <Table.Cell>
                        ...
                        <Table.Cell>
                            { (!vehicle.dateRelieved) && moment(vehicle.dateRelieved).format('MMM Do YYYY') }
                        </Table.Cell>
                    </Table.Row>)
...
export default graphql(assignedVehiclesFeed, {name:'assignedVehiclesFeed'}) (VehicleAssignmentList);