        ...
        const loadingMessage = (
          ...
          const timeoutMessage = (
            <Message icon negative>
              <Icon name="wait" />
              <Message.Content>
                <Message.Header>{this.props.RepairsRequisitionFeed.errorl}</Message.Header>
                Is the backend server running?
              </Message.Content>
            </Message>
          );
          if (this.props.RepairsRequisitionFeed && this.props.RepairsRequisitionFeed.loading) {
            return <div>{loadingMessage}</div>;
          }
      
          if (this.props.RepairsRequisitionFeed && this.props.RepairsRequisitionFeed.error) {
            return <div>{timeoutMessage}</div>;
          }
      
          if (this.props.RepairsRequisitionFeed.repairsRequestFeed === undefined ||this.props.RepairsRequisitionFeed.repairsRequestFeed=== 0) {
        ...
        return ( 
          ...
          <Table.Header>
            <Table.Row>
                  ...
                  <Table.Body>
                    {this.props.RepairsRequisitionFeed.repairsRequestFeed.map(request => (
                      <Table.Row>
                            <Table.Cell>{request.vehicleToBeRepaired.registrationNumber}</Table.Cell>
...
export default   graphql(Repairs_Requisition_Feed_Query,
    { name: 'RepairsRequisitionFeed' })(RepairsRequisitionsList);