...
import {Message, Menu, Button, Icon, Grid, Segment, Divider, Header, Table} from 'semantic-ui-react'
...
class MaterialsDetails extends Component {
    ...
    render() { 
        ...
        var RECIEPTS = transactions.filter(obj => obj.transactionType === 'RECIEPTS');
            ...
            <Grid textAlign="center" style={{ height: '100%' }} verticalAlign="middle">
                <Grid.Column style={{ maxWidth: 1600 }}>
                    ...
                    <b>Cost Per Unit:</b> {materialsCosting.costPerUnit} 
                        ...
                        <Divider />
                    <React.Fragment>
                    <b>Value of {materialsCosting.materialName[0].toUpperCase()+materialsCosting.materialName.slice(1).toLowerCase() } in Store:</b> {materialsCosting.costPerUnit * materialsCosting.balance.balance} 
                    </React.Fragment>
                        <Divider /> 
                        <React.Fragment>
                    ...
                    <Segment.Group horizontal >
                        ...
                        <Header as="h5" color="green" textAlign="center">
                                {materialsCosting.materialName[0].toUpperCase()+materialsCosting.materialName.slice(1).toLowerCase()} Recieved
                                        </Header>
                    ...
                    <Table.Header>
                                ...
                                <Table.Body>
                        {
                            ...
                            <Table.Row>
                                                <Table.Cell>{request.materialsRecievedFrom.supplierName}</Table.Cell>
                                                ...
                                                <Table.Cell>{moment(request.transactionDate).format('MMM Do YYYY')}</Table.Cell> 
                ...
                <Menu floated="right" pagination>
                  ...
                  <Menu.Item as="a" icon>
                    <Icon name="chevron right" />
                        ...
                        <Segment>
                        <Header as="h5" color="green" textAlign="center">
                                {materialsCosting.materialName[0].toUpperCase()+materialsCosting.materialName.slice(1).toLowerCase()} Issued
                                        </Header>
                        <Table celled selectable>
                                <Table.Header>
                        ...
                        <Table.Body>
                        {
                            ISSUES.map(request => (
                            <Table.Row>
                                                <Table.Cell>{request.materialsIssuedTo.projectName}</Table.Cell>
                                                <Table.Cell>{request.balanceBefore}</Table.Cell>  
                                                <Table.Cell>{request.unitsTransacted}</Table.Cell> 
                                                <Table.Cell>{request.balanceAfter}</Table.Cell> 
                                                <Table.Cell>{moment(request.transactionDate).format('MMM Do YYYY')}</Table.Cell> 
