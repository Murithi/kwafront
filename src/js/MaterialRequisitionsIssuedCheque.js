...
class MaterialRequisitionsIssuedCheque extends Component {
    ...
    if (this.props.requisitionFeed && this.props.requisitionFeed.loading) {
      ...
      if (this.props.requisitionFeed.issuedMaterialRequisitionsFeed === undefined ||this.props.requisitionFeed.issuedMaterialRequisitionsFeed.length === 0) {
                            ...
                            this.props.requisitionFeed.issuedMaterialRequisitionsFeed.map(request => {
                                if (request.otherPaymentDetails !== null) {
                                   return (<Table.Row>
                                        ...
                                        <Table.Cell>{currencyFormatter.format(request.approxCost, { code: 'KES' })}</Table.Cell>
                                        ...
                                        <Table.Cell>{currencyFormatter.format(request.otherPaymentDetails.amountPaid, { code: 'KES' })}</Table.Cell>
                                        <Table.Cell>{currencyFormatter.format(request.otherPaymentDetails.discountRecieved, { code: 'KES' })}</Table.Cell>
                                        <Table.Cell>{moment(request.otherPaymentDetails.datePaid).format('MMM Do YYYY')}</Table.Cell>
